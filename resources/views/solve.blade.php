...
<script>
  ...
  let isPdf = false;
  let compressedDataURL = null; // for images
  let history = [];
  ...
  async function handleFile(e) {
    ...
    if (isPdf) {
      preview.innerHTML = `
        ...
        <p class="text-xs text-gray-500 mt-1">This PDF will be sent directly to AI for reading (more accurate).</p>
      `;
  ...
  async function onSolve() {
    ...
    try {
      let data;
      if (isPdf) {
        // PDF：用 FormData 直传（后端会把 PDF 直接交给 GPT 文件输入）
        const fd = new FormData();
        fd.append('mode', 'direct_pdf');
        fd.append('pdf', selectedFile, selectedFile.name);
        data = await postFormData('/api/solve', fd, { timeoutMs: 60000 });
      } else {
  ...
  async function postFormData(url, formData, { timeoutMs = 60000 } = {}) {
